...
fn runMacos(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var mac_platform = try zero_native.platform.macos.MacPlatform.initWithOptions(
        zero_native.geometry.SizeF.init(1280, 900),
        webEngine(),
        app_info,
    );
    defer mac_platform.deinit();

    try runRuntime(mac_platform.platform(), app, options, store, init, app_info);
}
...
fn runLinux(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var linux_platform = try zero_native.platform.linux.LinuxPlatform.initWithOptions(
        zero_native.geometry.SizeF.init(1280, 900),
        webEngine(),
        app_info,
    );
    ...
    try runRuntime(linux_platform.platform(), app, options, store, init, app_info);
}
...
fn runWindows(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var windows_platform = try zero_native.platform.windows.WindowsPlatform.initWithOptions(
        zero_native.geometry.SizeF.init(1280, 900),
        webEngine(),
        app_info,
    );
    ...
    try runRuntime(windows_platform.platform(), app, options, store, init, app_info);
}
...
fn runNull(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    try runRuntime(null_platform.platform(), app, options, store, init, app_info);
}
...
fn runRuntime(
    ...
    var runtime = zero_native.Runtime.init(.{
        ...
        .automation = if (build_options.automation)
            zero_native.automation.Server.init(init.io, ".zig-cache/zero-native-automation", app_info.app_name)
        else
            null,
